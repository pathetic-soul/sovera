...
class PySandbox(Tool):
    ...
    def run(self, args: dict[str, Any], ctx: RunContext) -> ToolResult:
        ...
        with tempfile.TemporaryDirectory(prefix="sovereign-src-") as src:
            ...
            # A per-run name, not left to Docker's autogenerated one, is what
            # lets the operator kill switch (core/api/agent.py) target this
            # exact container with `docker kill` instead of waiting out
            # TIMEOUT_S — the whole point of a kill switch is not waiting.
            name = f"sovereign-sandbox-{uuid.uuid4().hex[:12]}"
            argv = [
                "docker", "run", "--name", name, *HARDENING,
            ...
            try:
                ...
                # An operator `docker kill` mid-run makes `docker run --rm`
                # exit on its own with a non-zero code — handled below as an
                # ordinary failed exit, not a special case. This only has to
                # stop a *future* step from seeing a stale, already-dead name.
                ctx.active_container = None
